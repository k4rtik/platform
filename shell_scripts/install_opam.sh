...
function create_opam_repo {
	if ! opam repo set-url "${COQ_PLATFORM_REPO_NAME}.patch_$2" "file://$OPAMPACKAGES/$1" 2>/dev/null; then
...
if ! command -v opam &>/dev/null; then
	...
	if [ $(version_to_number $(opam --version)) -lt $(version_to_number 2.0.5) ]; then
...
if [[ "$OSTYPE" == linux* ]]; then
	if ! command -v bwrap &>/dev/null; then
		...
		if [ $(version_to_number $(bwrap --version | cut -f 2 -d ' ')) -lt $(version_to_number 0.2.1) ]; then
			echo "========================= BUBBLEWRAP SYSTEM SANDBOX ========================="
...
if [ "${COQ_PLATFORM_USE_DEV_REPOSITORY}" == 'Y' ]; then
...
if ! opam switch list --short | fgrep -qx "${COQ_PLATFORM_SWITCH_NAME}" &>/dev/null; then
	...
	if [[ "$OSTYPE" == cygwin ]]; then
		if [ "$(uname -m)" = "x86_64" ]; then
			...
			*)
				echo "Unsupported OCaml version ${COQ_PLATFORM_OCAML_VERSION}"
				return 1
				;;
			esac
		else
...
if [ "$OSTYPE" == cygwin ] && [ "$BITSIZE" == 32 ] && [ "$(opam var arch)" != i686 ]; then
...
if [ ! -f "$HOME/.opam_update_timestamp" ] || [ $(find "$HOME/.opam_update_timestamp" -mmin +60 -print) ]; then
...
if [ ! -z ${SNAPCRAFT_PROJECT_NAME+x} ]; then
