...
for package in ${PACKAGES}; do
	...
	*) continue ;;
	...
	if opam_is_installed "${package_main}"; then
		...
		if [[ "${package_opam_version}" =~ ~flex$ ]]; then
	...
	if [[ "${package_dev_repo_url}" =~ ^git\+ ]]; then
