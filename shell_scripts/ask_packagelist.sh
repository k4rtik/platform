...
if [ -z "${COQ_PLATFORM_PACKAGE_PICK_FILE:+x}" ]; then
	cat <<EOH
...
EOH
	...
	packageindex=0
	for packagefile in ${packagefile_list}; do
		if ! grep -q "COQ_PLATFORM_VERSION_SORTORDER=" "$packagefile"; then
			echo "ERROR: package file '$packagefile' does not contain 'COQ_PLATFORM_VERSION_SORTORDER' definition."
			fail
		fi
		if ! grep -q "COQ_PLATFORM_VERSION_TITLE=" "$packagefile"; then
			echo "ERROR: package file '$packagefile' does not contain 'COQ_PLATFORM_VERSION_TITLE' definition."
			fail
		fi
...
EOH
	...
	for packagefile in ${packagefile_list}; do
		packageindex=$((${packageindex} + 1))
		if [ "${packageindex}" -eq "${ANSWER}" ]; then
